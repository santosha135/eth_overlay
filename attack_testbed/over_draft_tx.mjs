...
//const valueInEther = "999999992970600000000000000";  // Amount to send in Ether (e.g., 0.1 ETH)
const valueInEther =  web3.eth.getGasPrice()
const gasLimit = 21000;      // Gas limit for a simple transfer (standard ETH transfer)
const gasPriceInGwei = 20;   // Gas price in Gwei

const gasCost = web3.utils.toWei((gasPriceInGwei * gasLimit).toString(), 'gwei');
console.log("Balance: ", balanceWei.toString() );  // Debugging
console.log("Gas: ", gasCost); 

const amountToSend = balanceWei  - BigInt(gasCost);
...
async function sendTransaction() {
  try {
    ...
    const numberOfTransactions =1;  // Set how many transactions you want to send
    for (let i = 0; i < numberOfTransactions; i++) {
      // Explicitly convert everything to string
      const tx = {
        from: fromAddress,
        to: toAddress,
        value: amountToSend.toString(),  // Ensure value is passed as string
