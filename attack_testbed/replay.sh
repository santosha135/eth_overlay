...
apt-get update -y
apt-get install -y python3 python3-venv python3-pip vim nodejs npm curl procps
...
for i in $(seq 1 10); do
  test -f "replay_tx_v${i}.py" || {
    echo "Missing replay_tx_v${i}.py"
    exit 1
  }
done

echo "All replay_tx_v1.py ... replay_tx_v10.py exist."
'
...
for i in $(seq 1 10); do
  script="replay_tx_v${i}.py"
  log="logs/replay_tx_v${i}.log"

  echo "[START] $script"
  nohup python3 "$script" > "$log" 2>&1 &
  # nohup ./venv/bin/python -u "$script" > "$log" 2>&1 &
  echo $! > "logs/replay_tx_v${i}.pid"
done
...
echo "cd $REMOTE_DIR && tail -f logs/replay_tx_v*.log"