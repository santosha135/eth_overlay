...
NORMAL_FILES = [
    Path(
        f"replay_phased_fixed_metrics_replay_tx_v{i}_sm_normal.xlsx"
    )
...
# ------------------------------------------------------------
# Modified: v1 to v10 for each bucket configuration
# ------------------------------------------------------------
...
MODIFIED_FILES = {
    ...
    "Modified - 5 Buckets": [
        Path(
            f"replay_phased_fixed_metrics_replay_tx_v{i}_modified_5.xlsx"
        )
        for i in range(1, NUMBER_OF_RUNS + 1)
    ],
    ...
    "Modified - 10 Buckets": [
        Path(
            f"replay_phased_fixed_metrics_replay_tx_v{i}_modified_10.xlsx"
        )
        for i in range(1, NUMBER_OF_RUNS + 1)
    ],
    ...
    "Modified - 15 Buckets": [
        Path(
            f"replay_phased_fixed_metrics_replay_tx_v{i}_modified_15.xlsx"
        )
        for i in range(1, NUMBER_OF_RUNS + 1)
    ],
    ...
    "Modified - 30 Buckets": [
        Path(
            f"replay_phased_fixed_metrics_replay_tx_v{i}_modified_30.xlsx"
        )
        for i in range(1, NUMBER_OF_RUNS + 1)
    ],
}
...
def load_multiple_runs(
    ...
    """
    Load and combine v1 ... v10 for one experiment.

    Example:
        Normal v1-v10
        Modified 5 Buckets v1-v10
    """

    combined_dfs = []

    combined_filter_information = {
        "total_rows": 0,
    ...
    for run_number, file_path in enumerate(
        file_paths,
        start=1,
    ):

        print(
            f"Run v{run_number}: {file_path.name}"
        )

        df, filter_information = load_latency(
            file_path=file_path,
            case_name=case_name,
        )

        # Remember which experimental run this transaction came from.
        df["run"] = run_number

        combined_dfs.append(df)

        # Add filtering counts from this run.
        for key in combined_filter_information:
            combined_filter_information[key] += (
                filter_information[key]
            )

        print(
            f"    included transactions: {len(df):,}"
        )

    # ----------------------------------------------------------
    # Combine all v1-v10 vertically
    # ----------------------------------------------------------
    ...
    # New transaction number covering ALL ten runs.
    combined_df["transaction_number"] = np.arange(
        1,
        len(combined_df) + 1,
    )
    ...
    print(
        f"{case_name}: "
        f"{len(combined_df):,} total transactions "
        f"from {len(file_paths)} runs"
    )
...
def main() -> None:
    ...
    try:
        ...
        # We now have multiple files per case.
        all_file_paths = {
            "Normal": NORMAL_FILES,
        }
        ...
        # ======================================================
        # Load MODIFIED v1-v10 for every bucket configuration
        # ======================================================
        ...
        for case_name, file_paths in MODIFIED_FILES.items():
            ...
            case_df, case_filter_information = (
                load_multiple_runs(
                    file_paths,
                    case_name,
                )
        ...
        # ======================================================
        # Print combined filtering information
        # ======================================================

        print("\n")
        print("=" * 100)
        print("COMBINED FILTERING RESULTS — V1 TO V10")
        print("=" * 100)

        for case_name, df in all_cases.items():

            info = all_filter_information[case_name]
            ...
            print(
                f"Number of runs                  : "
                f"{len(all_file_paths[case_name])}"
            )
            ...
            print(
                f"Invalid/failed rows             : "
                f"{info['invalid_or_unsuccessful_rows']:,}"
            )

            print(
                f"Negative latency excluded       : "
                f"{info['negative_latency_rows']:,}"
            )

            print(
                f"Latency > {MAX_LATENCY_SECONDS:.0f}s excluded      : "
                f"{info['above_max_latency_rows']:,}"
            )
        ...
        print("\n")
        print("=" * 100)
        print("COMBINED LATENCY STATISTICS — V1 TO V10")
        print("=" * 100)
        ...
        # ======================================================
        # Normal versus modified comparison
        # ======================================================
        ...
        for case_name, df in all_cases.items():
            ...
            print(
                f"{case_name:<25} "
                f"{len(df):>10,} transactions"
            )

        print(
            "\nEach distribution contains the "
            "combined transactions from v1-v10."
        )

         # ======================================================
        # Check latency tail after ALL cases have been loaded
        # ======================================================

        print("\n" + "=" * 100)
        print("LATENCY TAIL CHECK")
        print("=" * 100)
        ...
        for case_name, df in all_cases.items():
            ...
            print(f"Transactions : {len(latency):,}")
            print(f"Minimum      : {latency.min():.3f} s")
            print(f"Median       : {latency.median():.3f} s")
            print(f"P90          : {latency.quantile(0.90):.3f} s")
            print(f"P95          : {latency.quantile(0.95):.3f} s")
            print(f"P99          : {latency.quantile(0.99):.3f} s")
            print(f"P99.9        : {latency.quantile(0.999):.3f} s")
            print(f"Maximum      : {latency.max():.3f} s")

            for threshold in [
                14, 15, 20, 30, 60, 100, 300, 600
            ]:
                count = int((latency > threshold).sum())
                percent = (
                    count / len(latency) * 100.0
                )

                print(
                    f"> {threshold:3d} sec : "
                    f"{count:6,d} ({percent:.4f}%)"
                )
