...
print("Equal sample size used for both methods:", N_EQUAL)

# Keep all runtime samples if it is the smaller group
runtime_values = runtime_values.sample(
    n=N_EQUAL,
    random_state=42
).reset_index(drop=True)

# Randomly select the same number from regular execution
regular_values = regular_values.sample(
    n=N_EQUAL,
    random_state=42
).reset_index(drop=True)
