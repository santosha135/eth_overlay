...
MODIFIED_FILES = [
    f"replay_phased_fixed_metrics_replay_tx_v{i}_sm_modified_censor.xlsx"
    for i in range(1, 11)
]

NORMAL_FILES = [
    f"replay_phased_fixed_metrics_replay_tx_v{i}_sm_normal_censor.xlsx"
    for i in range(1, 11)
]
...
def load_target_address_latency(
    ...
    # ========================================================
    # Read v1 ... v10 and combine into ONE dataframe
    # ========================================================
    ...
    for run_number, file_path in enumerate(file_paths, start=1):
        ...
        # Keep track of which run each transaction came from
        df_run["run"] = "v{}".format(run_number)
        ...
        all_dataframes.append(df_run)

    # Combine v1-v10 vertically
    df = pd.concat(
        all_dataframes,
        ignore_index=True,
    )
    ...
    print()
    print(
        "TOTAL rows after combining v1-v10: {:,}".format(
            len(df)
        )
    )
